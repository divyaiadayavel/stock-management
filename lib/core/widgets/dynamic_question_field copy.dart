...
class _DynamicQuestionFieldState extends State<DynamicQuestionField> {
  ...
  // VALIDATION
  //
  // Every rule lives in Validators, same as the builder screens —
  // this is just picking the right one based on what the question
  // is actually asking for. A "name"-ish label needs at least 3
  // letters, a "phone"-ish label needs exactly 10 digits, and so on.
  // ─────────────────────────────────────────────────────────────
  String? _validateShortAnswer(ServiceQuestionEntity q, String? rawValue) {
    ...
    if (label.contains('name') ||
      ...
      // typing, the usual "at least 3 letters" name rule applies.
  ...
  String? _validateParagraph(ServiceQuestionEntity q, String? rawValue) {
    ...
    switch (q.type) {
      case QuestionType.shortAnswer:
        return TextFormField(
          ...
          keyboardType:
              q.label.toLowerCase().contains('phone') ||
          ...
          validator: (v) => _validateShortAnswer(q, v),
      ...
      case QuestionType.paragraph:
        return TextFormField(
          ...
          validator: (v) => _validateParagraph(q, v),
          onChanged: widget.onChanged,
        );
