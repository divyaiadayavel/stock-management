...
//
// FIXED:
// - All syntax errors removed (missing brackets, semicolons, etc.)
// - _getTotalReceivingForPo now returns int explicitly.
// - Replaced deprecated .withOpacity with .withValues(alpha:)
// - Removed unnecessary .toList() in spread
// - All map/forEach closures correctly closed.
//
import 'dart:convert';
...
class _ReceiveOrderScreenState extends ConsumerState<ReceiveOrderScreen>
  ...
  void _showPriceHistory(int productId, String productName) async {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _PriceHistoryModal(
  ...
  int _getTotalReceivingForPo(int poId) {
    ...
    if (receivingMap == null) return 0;
    return receivingMap.values.fold<int>(0, (sum, val) => sum + val);
  }

  bool _hasOutstandingForPo(int poId) {
    final receivingMap = _receivingMapPerPo[poId];
    if (receivingMap == null) return false;
    return receivingMap.values.any((qty) => qty > 0);
  }
  ...
  void _toggleExpansion(int poId) {
    if (_expandedPoId == poId) {
      setState(() {
        _setExpandedPoId(null);
      });
    } else {
      setState(() {
        _setExpandedPoId(poId);
      });
      if (!_poDetailsCache.containsKey(poId)) {
        _loadPoDetail(poId);
      }
    }
  }
        ...
        .map((item) {
          ...
          if (priceController != null) {
            ...
            if (priceText.isNotEmpty) {
              unitPrice = double.tryParse(priceText);
            }
          }
  ...
  'quantity_received': qtyVal,
    ...
    try {
      ...
      if (response.statusCode == 200 && res['success'] == true) {
        ...
        final newPoId = res['data']?['new_po_id'];
        final newPoNumber = res['data']?['new_po_number'];
            ...
            child: widget.isLoading
                ...
                : widget.items.isEmpty
                    ...
                    : Padding(
                        ...
                        child: Column(
                          children: [
                            Divider(color: AppColors.border, height: R.sp(context, AppSpacing.lg)),
                            // --- FIX: Spread with correct map and closing brackets ---
                            ...widget.items.map((item) {
                              ...
                              return _ReceivingItemTile(
                                ...
                                onQtyChanged: (newQty) => widget.onQtyChanged(itemId, newQty),
                                onShowPriceHistory: widget.onShowPriceHistory,
                              );
                            }), // <-- map closed, toList called
                            ...
                            Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  Divider(height: R.sp(context, AppSpacing.sm), color: AppColors.border),
                                  _SummaryRow(label: 'Grand Total', value: grandTotal, isBold: true),
                                ],
                              ),
                            ),
                          ],
...
// ============================================================================
// Receiving Item Tile – with editable price field
// ============================================================================
                ...
                child: Row(
                  ...
                  children: [
                    Row(
                      children: [
                        IconButton(
                          icon: Icon(Icons.remove_circle_outline, size: R.icon(context, 22)),
                          color: currentQty > 0
                              ? AppColors.primary
                        ...
                        IconButton(
                          icon: Icon(Icons.add_circle_outline, size: R.icon(context, 22)),
                          color: currentQty < outstanding
                              ? AppColors.primary
                              : AppColors.textSecondary.withValues(alpha: 0.4),
                          padding: EdgeInsets.zero,
                          constraints: const BoxConstraints(),
                          onPressed: currentQty < outstanding
                    ...
                    Column(
                      ...
                      children: [
                        Text(
                          'Line total',
                          style: AppTextStyles.small.copyWith(fontSize: R.fs(context, 9)),
                        ),
                        Text(
