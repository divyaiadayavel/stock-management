...
import '../widgets/payable_supplier_card.dart';
import 'supplier_details_screen.dart';

class SupplierPayablesListScreen extends ConsumerStatefulWidget {
  const SupplierPayablesListScreen({super.key});

  @override
  ConsumerState<SupplierPayablesListScreen> createState() =>
      _SupplierPayablesListScreenState();
}

class _SupplierPayablesListScreenState
    extends ConsumerState<SupplierPayablesListScreen> {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: AppBar(
          ...
          title: Text('Payable to Suppliers', style: AppTextStyles.heading),
          actions: [
            IconButton(
              icon: Icon(
      ...
      body: Column(
        children: [
          ...
          // ── LIST BODY ──
          Expanded(
            child: state.payables.isEmpty
                ? Center(
                    child: Text(
                      'No pending supplier payments for this period',
                ...
                : RefreshIndicator(
                    ...
                    child: ListView.separated(
                      ...
                      itemCount: state.payables.length,
                      separatorBuilder: (_, _) =>
                          SizedBox(height: R.sp(context, AppSpacing.sm)),
                      itemBuilder: (context, index) {
                        final supplier = state.payables[index];
                        return PayableSupplierCard(
                          supplier: supplier,
...
class _PayableFilterChip extends StatelessWidget {
  final String label;
  final bool selected;
  final VoidCallback onTap;

  const _PayableFilterChip({
    required this.label,
